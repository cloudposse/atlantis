	...
	if !hasConfigFile {
		...
		for _, mp := range modifiedProjects {
			projCtxs = append(projCtxs, models.ProjectCommandContext{
				...
				CustomStageCmds: p.CommentBuilder.BuildCustomStageComments(mp.Path, DefaultWorkspace, ""),
			})
		}
	} else {
		...
		for i := 0; i < len(matchingProjects); i++ {
			...
			projCtxs = append(projCtxs, models.ProjectCommandContext{
				...
				CustomStageCmds: p.CommentBuilder.BuildCustomStageComments(mp.Dir, mp.Workspace, mp.GetName()),
			})
		}
	}
	return projCtxs, nil
...
func (p *DefaultProjectCommandBuilder) buildProjectCommandCtx(ctx *CommandContext, projectName string, commentFlags []string, repoDir string, repoRelDir string, workspace string) (models.ProjectCommandContext, error) {
	...
	return models.ProjectCommandContext{
		...
		CustomStageCmds: p.CommentBuilder.BuildCustomStageComments(repoRelDir, workspace, projectName),
	}, nil
}
